...
class ReviewsController extends Controller
{
    ...
    public function index()
    {
        ...
        {
             if($user_group_id == 0){
                $data['reviews'] = Reviews::with(['hasOneCustomer','hasOneOrder'])->orderBy('store_review_id','DESC')->get();
             }
             else{
                $data['reviews'] = Reviews::with(['hasOneCustomer','hasOneOrder'])->where('store_id',$current_store_id)->orderBy('store_review_id','DESC')->get();
             }
        }
        else
        {
            $data['reviews'] = Reviews::with(['hasOneCustomer','hasOneOrder'])->where('store_id',$user_shop_id)->orderBy('store_review_id','DESC')->get();
        }
