...
class OrdersController extends Controller
{
    ...
    public function addneworders(Request $request)
    {
        ...
        if(!empty($total) && !empty($usercart) && count($usercart)>0 )
        {
        ...
        {
            return redirect()->route('ordersinsert')->with('empty_error','Please Select Products !');
        }
    ...
    {
        $select_store_id = $request->select_store_id;

        $res = Customer::where('firstname', 'LIKE', "%{$request->term}%")->where('store_id', $select_store_id)->get();
    ...
    public function autocompleteproduct(Request $request)
    {
        ...
        $pro = ProductDescription::with(['hasOneProductToStore'])->whereHas('hasOneProductToStore', function ($q) use ($select_store_id) {
            $q->where('store_id', $select_store_id);
